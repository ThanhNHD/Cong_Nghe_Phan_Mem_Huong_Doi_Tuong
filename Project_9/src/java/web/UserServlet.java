...
public class UserServlet extends HttpServlet {
	...
	private void viewContentForm(HttpServletRequest request, HttpServletResponse response)
		...
		if (numId == 0) {
			...
			if (numPage >= 1) {
				...
				dispatcher = request.getRequestDispatcher("view-content.tiles?Page=0&numId=10" + NumPageButton);
				dispatcher.forward(request, response);
			} else {
	...
	private void deleteContent(HttpServletRequest request, HttpServletResponse response)
		...
		if (id != 1)
			...
			response.sendRedirect("home-page.tiles");
			}
	...
	/*
	 * private void updateProfileForm(HttpServletRequest request,
	 * HttpServletResponse response) throws ServletException, IOException {
	 * response.setContentType("text/html;charset=UTF-8");
	 * request.setCharacterEncoding("UTF-8"); String checker =
	 * request.getParameter("name"); int authorid =
	 * Integer.parseInt(request.getParameter("id")); User user =
	 * userDAO.selectUserBYidname(authorid,checker); if(user == null) {
	 * RequestDispatcher dispatcher =
	 * request.getRequestDispatcher("home-page.tiles?id="+authorid+"&name="+checker)
	 * ; dispatcher.forward(request, response); } else { RequestDispatcher
	 * dispatcher =
	 * request.getRequestDispatcher("edit-profile.tiles?id="+authorid+"&name="+
	 * checker+"&lname="+user.getLname()+"&fname="+user.getFname()+"&phone="+user.
	 * getPhone()+"&email="+user.getEmail()+"&des="+user.getDescription());
	 * dispatcher.forward(request, response); } }
	 */
